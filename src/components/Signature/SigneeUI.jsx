...
import { User, Settings, LogOut, UserCircle, PenTool, Type, FileSignature, X, Upload, Palette } from 'lucide-react';
...
const SignatureModal = ({ isOpen, onClose, fieldType, onSave }) => {
  ...
  const fonts = [
    ...
    { name: 'Sans-serif', value: 'sans-serif', style: { fontFamily: 'sans-serif' } },
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          {activeTab === 'text' && (
            <div className="space-y-6">
              <div>
                ...
                <input
                  type="text"
                  value={fieldType === 'initials' ? textInitials : textSignature}
                  onChange={(e) => fieldType === 'initials' ? setTextInitials(e.target.value) : setTextSignature(e.target.value)}
                  ...
                  placeholder={fieldType === 'initials' ? 'Enter your initials' : 'Enter your full name'}
                />
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {fonts.map((font) => (
                    ...
                    >
                      <div className="text-sm font-medium text-gray-700 mb-2">{font.name}</div>
                      <div 
          ...
          {activeTab === 'upload' && (
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-3">
                  Upload an image of your {fieldType === 'initials' ? 'initials' : 'signature'}
                </label>
...
const SignatureField = ({ field, onFieldClick, canvasWidth, canvasHeight, signeeColor, isCompleted }) => {
  const getFieldIcon = () => {
    switch (field.type) {
      case 'signature':
  ...
  const renderFieldContent = () => {
    ...
    if (isCompleted && field.signatureData) {
      ...
      } else if (field.signatureData.type === 'draw' || field.signatureData.type === 'upload') {
        return (
...
const SigneeUI = () => {
  ...
  const handleSignatureSave = (signatureData) => {
    if (selectedField) {
      ...
      const newCompletedFields = new Set([...completedFields, selectedField.id]);
