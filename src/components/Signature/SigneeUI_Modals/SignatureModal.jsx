...
const SignatureModal = ({ isOpen, onClose, onSave, defaultReason = "" }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl h-[90vh] max-h-[800px] overflow-hidden border border-gray-200">
        {/* Header */}
        <div className="bg-gradient-to-r from-CloudbyzBlue to-blue-600 text-white p-3 sm:p-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg sm:text-xl font-bold">Create Your Signature</h2>
              <p className="text-blue-100 text-xs sm:text-sm">Design a professional digital signature</p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="flex h-[calc(90vh-80px)] max-h-[720px]">
          {/* Left Panel - Signature Creation */}
          <div className="flex-1 flex flex-col min-w-0">
            <div className="p-3 sm:p-4 flex-1 overflow-y-auto">
              {/* Reason Selection */}
              <div ref={dropdownRef} className="mb-3 sm:mb-4">
                <label className="block text-xs sm:text-sm font-semibold text-gray-700 mb-2 flex items-center">
                  <MousePointer className="w-3 h-3 sm:w-4 sm:h-4 mr-1 text-CloudbyzBlue" />
                  Reason to Sign *
                </label>
                  ...
                  {isCustomReason ? (
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 border border-gray-300 rounded-lg focus:border-CloudbyzBlue focus:ring-1 focus:ring-CloudbyzBlue transition-all p-2 text-xs sm:text-sm"
                        maxLength={50}
                      />
                      <button
                        onClick={handleCustomReasonSave}
                        className="bg-CloudbyzBlue text-white rounded-lg hover:bg-blue-600 transition-colors px-2 sm:px-3 py-2 text-xs sm:text-sm font-medium"
                      >
                        Save
                      </button>
                      <button
                        ...
                        className="bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition-colors px-2 sm:px-3 py-2 text-xs sm:text-sm font-medium"
                      >
                        Cancel
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-between bg-white border border-gray-300 rounded-lg hover:border-gray-400 focus:border-CloudbyzBlue focus:ring-1 focus:ring-CloudbyzBlue transition-all p-2 text-xs sm:text-sm"
                    >
                      <span className={selectedReason ? "text-gray-800" : "text-gray-500"}>
                        {selectedReason || "Select a reason to sign"}
                      </span>
                      <ChevronDown className={`w-3 h-3 sm:w-4 sm:h-4 text-gray-500 transition-transform ${showReasonDropdown ? "rotate-180" : ""}`} />
                    </button>
                  )}
                    ...
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-[70] max-h-48 overflow-y-auto">
                      {signatureReasons.map((reason, index) => (
                        <button
                          ...
                          className="w-full text-left hover:bg-gray-50 transition-colors p-2 text-xs sm:text-sm"
                        >
                          {reason}
                        </button>
                      ))}
                      ...
                      <div className="border-t border-gray-200">
                        <button
                          ...
                          className="w-full text-left hover:bg-blue-50 transition-colors text-CloudbyzBlue font-medium p-2 text-xs sm:text-sm"
                        >
                          Other reason...
                        </button>
                      </div>
                ...
                <div className="flex border-b border-gray-200">
                  {[
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-2 font-medium transition-colors text-xs sm:text-sm ${
                        activeTab === tab.id
              ...
              <div className="flex-1 bg-gray-50 rounded-lg p-3 sm:p-4">
                {activeTab === "draw" && (
                  <div className="h-full flex flex-col">
                    ...
                    <div className="flex-1 flex flex-col">
                      <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">Draw Your Signature</label>
                      <div className="flex-1 border-2 border-dashed border-gray-300 rounded-lg bg-white p-2 min-h-[120px] sm:min-h-[150px]">
                        <canvas
                          ref={canvasRef}
                          width={500}
                          height={150}
                      ...
                      <div className="flex justify-end mt-2">
                        <button
                          onClick={clearCanvas}
                          className="text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors px-2 sm:px-3 py-1 text-xs sm:text-sm"
                        >
                          Clear
                        </button>
                      </div>
                ...
                {activeTab === "upload" && (
                  <div className="h-full flex flex-col">
                    <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">Upload Signature Image</label>
                    <div className="flex-1 border-2 border-dashed border-gray-300 rounded-lg text-center bg-white p-4 min-h-[120px] sm:min-h-[150px] flex flex-col items-center justify-center">
                      {uploadedImage ? (
                        <div>
                          <img
                            src={uploadedImage}
                            alt="Uploaded signature"
                            className="mx-auto mb-3 max-h-16 sm:max-h-24"
                          />
                          <button
                        ...
                        <div>
                          ...
                          <p className="text-gray-600 mb-3 text-xs sm:text-sm">Click to upload or drag and drop</p>
                          <input
                            ref={fileInputRef}
                            type="file"
                            accept="image/*"
                          ...
                          <button
                            ...
                            className="bg-CloudbyzBlue text-white rounded-lg hover:bg-blue-600 transition-colors px-3 sm:px-4 py-2 text-xs sm:text-sm"
                          >
                            Choose File
                          </button>
                        </div>
                ...
                {activeTab === "text" && (
                  <div className="h-full flex flex-col">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                      <div>
                        <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">Text</label>
                        <input
                          type="text"
                          value={typedSignature}
                          onChange={(e) => setTypedSignature(e.target.value)}
                          className="w-full border border-gray-300 rounded-lg focus:border-CloudbyzBlue focus:ring-1 focus:ring-CloudbyzBlue transition-all p-2 text-xs sm:text-sm"
                          placeholder="Enter your name"
                        />
                      </div>
                      <div>
                        <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">Font</label>
                        <select
                          value={selectedFont}
                      ...
                      <div className="flex gap-2">
                        {colors.map((color) => (
                          <button
                            key={color}
                            onClick={() => setSelectedColor(color)}
                            className={`w-4 h-4 sm:w-5 sm:h-5 rounded border-2 transition-all ${
                              selectedColor === color
                                ? "border-gray-800 scale-110"
                                : "border-gray-300"
                            }`}
                    ...
                    <div className="flex-1">
                      ...
                      <div className="h-full border border-gray-300 rounded-lg bg-white text-center flex items-center justify-center min-h-[80px] sm:min-h-[100px]">
                        <span
                          style={{
            ...
            <div className="p-3 sm:p-4 border-t border-gray-200 bg-gray-50">
              <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
                <button
                  ...
                  className="bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium flex items-center justify-center gap-2"
                >
                ...
                <button
                  ...
                  className="bg-CloudbyzBlue text-white rounded-lg hover:bg-blue-600 transition-colors px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium flex items-center justify-center gap-2"
                >
