...
const SignatureLibrary = ({ 
  ...
  return (
    <div className="w-64 sm:w-72 bg-white border-l border-gray-200 flex flex-col h-full">
      {/* Header */}
      <div className="p-3 sm:p-4 border-b border-gray-200 bg-gray-50">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <Library className="w-4 h-4 sm:w-5 sm:h-5 text-CloudbyzBlue" />
            <h3 className="font-semibold text-gray-800 text-sm sm:text-base">Signature Library</h3>
          </div>
        ...
        {savedSignatures.length === 0 ? (
          <div className="text-center py-6 sm:py-8">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-3">
              <PenTool className="w-5 h-5 sm:w-6 sm:h-6 text-gray-400" />
            </div>
            <p className="text-xs sm:text-sm text-gray-500 mb-1">No signatures yet</p>
          ...
          <div className="space-y-2">
            {savedSignatures.map((signature) => {
              ...
              return (
                <div
                  key={signature.id}
                  className={`relative group border rounded-lg p-2 sm:p-3 cursor-pointer transition-all duration-200 ${
                    isSelected
                ...
                >
                  {/* Signature Preview */}
                  <div className="h-10 sm:h-12 mb-2 bg-gray-50 rounded border flex items-center justify-center overflow-hidden">
                    <img
                      src={signature.data}
                      alt="Saved signature"
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <div className={`w-3 h-3 sm:w-4 sm:h-4 rounded ${getTypeColor(signature.type)} flex items-center justify-center`}>
                  ...
                  <button
                    ...
                    className="absolute top-1 sm:top-2 right-1 sm:right-2 w-5 h-5 sm:w-6 sm:h-6 bg-red-500 hover:bg-red-600 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200"
                  ...
                  {/* Selection Indicator */}
                  {isSelected && (
                    <div className="absolute -top-1 -right-1 w-4 h-4 sm:w-5 sm:h-5 bg-CloudbyzBlue rounded-full flex items-center justify-center">
